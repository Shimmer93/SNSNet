...
class JBFCocoMetric(BaseMetric):
    ...
    def process(self, data_batch: Sequence[dict],
        ...
        """
        # print(data_samples[0]['pred_fields'].keys(), data_samples[0].keys())
        for data_sample in data_samples:
            ...
            if 'pred_fields' in data_sample:
                ...
                if self.vis_flag:
                    self.visualizer.visualize_jbf(f'vis/{self.count:03d}_{id}_{img_id}.png', img_path, \
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for pred in preds:
            ...
            for idx, keypoints in enumerate(pred['keypoints']):
                ...
                # print('scores:', pred['keypoint_scores'][idx].shape)
                if 'bbox' in pred:
            ...
            for instance in instances:
                ...
                if self.score_mode == 'bbox':
                    ...
                    if self.score_mode == 'bbox_rle':
                        ...
                        mean_kpt_score = 0
                        valid_num = 0
                        # print(len(instance['keypoint_scores']), num_keypoints)
    ...
    def results2jbfs(self, keypoints: Dict[int, list],
                     outfile_prefix: str) -> str:
        for _, img_kpts in keypoints.items():
            for i, instance in enumerate(img_kpts):
                img_id = instance['img_id']
                category_id = instance['category_id']
                masks = instance['masks']
                body_mask = masks[0].cpu().numpy()
                joint_mask = masks[1:].cpu().numpy()
                write_jbf(f'{outfile_prefix}_{img_id}_{category_id}_{i}.png', joint_mask, body_mask, rescale_ratio=4.)

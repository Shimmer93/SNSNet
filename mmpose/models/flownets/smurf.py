...
from torchvision.models.optical_flow._utils import (
    grid_sample,
    make_coords_grid,
    upsample_flow,
)
...
class RecurrentBlock(nn.Module):
    ...
    Takes the current hidden state and the concatenation of (motion encoder output, context) as input. Returns an
    ...
    def __init__(self, *, input_size, hidden_size, kernel_size=((1, 5), (5, 1)), padding=((0, 2), (2, 0))):
        ...
        if len(kernel_size) != len(padding):
            raise ValueError(
                f"kernel_size should have the same length as padding, instead got len(kernel_size) "
                f"= {len(kernel_size)} and len(padding) = {len(padding)}"
            )
...
class MaskPredictor(nn.Module):
    ...
    It takes the hidden state of the recurrent unit as input and outputs the mask. This is not used in the raft-small
    model.
    """
...
class CorrBlock(nn.Module):
    ...
    def build_pyramid(self, fmap1, fmap2):
        ...
        The correlation volume is first computed as the dot product of each pair (pixel_in_fmap1, pixel_in_fmap2) The
        last 2 dimensions of the correlation volume are then pooled num_levels times at different resolutions to build
        the correlation pyramid.
        """

        if fmap1.shape != fmap2.shape:
            raise ValueError(
                f"Input feature maps should have the same shape, instead got {fmap1.shape} "
                f"(fmap1.shape) != {fmap2.shape} (fmap2.shape)"
            )
    ...
    def index_pyramid(self, centroids_coords):
        centroids_coords = centroids_coords.permute(0, 2, 3, 1)
        b, h1, w1, _ = centroids_coords.shape
        out_pyramid = []
        for i, corr in enumerate(self.corr_pyramid):
            corr = corr.permute(0, 2, 3, 1)
            start = -float(self.radius)
            stop = float(self.radius)
            num = float(2 * self.radius + 1)

            dx = torch.linspace(int(start), int(stop), int(num)).to(centroids_coords.device)
            dy = torch.linspace(int(start), int(stop), int(num)).to(centroids_coords.device)
            delta = torch.stack(torch.meshgrid(dy, dx), dim=-1).flip(dims=(-1,))
            centroid_lvl = centroids_coords.reshape(b * h1 * w1, 1, 1, 2) / 2**i
            delta_lvl = delta.reshape(1, 2 * self.radius + 1, 2 * self.radius + 1, 2)
            coords_lvl = centroid_lvl + delta_lvl
            corr = grid_sample(corr.permute(0, 3, 1, 2), coords_lvl.clone(), align_corners=True).permute(0, 2, 3, 1)
            channel_dim = (2 * self.radius + 1) * (2 * self.radius + 1)
            corr = corr.reshape(b, h1, w1, channel_dim)
            out_pyramid.append(corr)
        out = torch.concat(out_pyramid, dim=-1).permute(0, 3, 1, 2)
        return out
...
class RAFT(nn.Module):
    ...
    def forward(self, image1, image2, num_flow_updates: int = 12):
        ...
        if out_channels_context <= 0:
            raise ValueError(
                f"The context encoder outputs {context_out.shape[1]} channels, but it should have at strictly "
                f"more than hidden_state={hidden_state_size} channels"
            )
...
class SMURF(nn.Module):
    def __init__(self, checkpoint=None):
        super(SMURF, self).__init__()
        self.model = raft_smurf(checkpoint)

    def forward(self, image1, image2, num_flow_updates=12):
        return self.model(image1, image2, num_flow_updates)
